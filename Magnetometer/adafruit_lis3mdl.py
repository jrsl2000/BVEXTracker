...
_LIS3MDL_TEMP_OUT_L = const(0x2E)
_LIS3MDL_TEMP_OUT_H = const(0x2F)
...
class LIS3MDL:
    ...
    **Quickstart: Importing and using the device**
    ...
    _temp_en = RWBit(_LIS3MDL_CTRL_REG1, 7)
    _temp_l = ROBits(8, _LIS3MDL_TEMP_OUT_L, 0)
    _temp_h = ROBits(8, _LIS3MDL_TEMP_OUT_H, 0)
    ...
    def temp_enable(self):
        return self.__temp_en
        
    @temp_enable.setter
    def temp_enable(self, enable):
        if (enable):
            self.__temp_en = 1
        else: 
            self.__temp_en = 0
    ...
    def get_temp(self) -> float:
        temp_l = self._temp_l
        temp_h = self._temp_h
        t = self._scale_temp_data(temp_l, temp_h)
        return t
    
    def _scale_temp_data(self, tl, th) -> float:
        temp_total = (th << 8) | tl
        if temp_total > 32767:
            temp_total = 65536
        ctemp = (temp_total * 0.125) + 25
        return ctemp
